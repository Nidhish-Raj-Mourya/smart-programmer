...
/** @type {Record<string, (title: string) => { problem: string; approach: string[]; solution: string; tip?: string }>} */
const BUILDERS = {
  'array-display': () => ({
    problem: 'User se array input lo aur poori array print karo — koi calculation nahi.',
    approach: ['arr = input se read karo', 'print(arr) se saari values ek saath dikhao'],
  ...
  'array-sum': () => ({
    ...
    approach: ['total = 0', 'Har i par: total += arr[i]', 'End par print(total)'],
  ...
  'array-average': () => ({
    ...
    approach: ['Pehle total sum nikalo (loop se)', 'avg = total / len(arr)', 'print(avg)'],
  ...
  'array-count-signs': () => ({
    ...
    approach: ['pos, neg, zero = 0', 'Har x: x>0 → pos++, x<0 → neg++, x==0 → zero++'],
    solution: 'Teen-way branch har element par. Single pass O(n).',
  }),
  'array-sum-evens': () => ({
    problem: 'Sirf even numbers ka sum chahiye.',
    approach: ['total = 0', 'Har x: agar x % 2 == 0 to total += x'],
  ...
  'array-product': () => ({
    ...
    approach: ['prod = 1 (sum ki jagah multiply)', 'Har element: prod *= arr[i]', 'Zero ho to product 0 ban jata hai'],
  ...
  'array-frequency-table': () => ({
    ...
    approach: ['freq = {}', 'Har x: freq[x] = freq.get(x,0) + 1', 'freq print'],
    solution: 'HashMap counting — key = value, value = count. O(n).',
  }),
  'array-max-frequency': () => ({
    problem: 'Sabse zyada baar aane wala element.',
    approach: ['Pehle frequency table banao', 'freq map par loop — max count wala key dhundho'],
  ...
  'array-reverse-inplace': () => ({
    ...
    approach: ['left = 0, right = n-1', 'While left < right: swap(arr[left], arr[right])', 'left++, right--'],
  ...
  'array-rotate-left-1': () => ({
    ...
    approach: ['first = arr[0] save karo', 'Sab elements ek index left shift: arr[i] = arr[i+1]', 'arr[n-1] = first'],
  ...
  'array-rotate-right-1': () => ({
    ...
    approach: ['last = arr[n-1]', 'Right se left: arr[i] = arr[i-1]', 'arr[0] = last'],
    solution: 'Right shift loop ulta chalao — overwrite se bachne ke liye last pehle save.',
  }),
  'array-rotate-left-k': () => ({
    problem: 'K positions left rotate.',
    approach: ['K = K % n (effective rotations)', 'Pehle K elements reverse, phir baaki reverse, phir poori reverse', 'Ya K baar rotate-left-1 (slow)'],
  ...
  'array-copy': () => ({
    ...
    approach: ['arr2 = [] ya same size array', 'Har i: arr2[i] = arr[i]', 'arr2 return/print'],
  ...
  'array-insert-at': () => ({
    ...
    approach: ['i = n-1 se pos tak: arr[i+1] = arr[i] (ULTA loop)', 'arr[pos] = val', 'Size +1'],
  ...
  'array-delete-at': () => ({
    ...
    approach: ['i = pos se n-2: arr[i] = arr[i+1]', 'Size kam (pop last)'],
    solution: 'Left shift gap band karta hai. O(n).',
  }),
  'array-insert-sorted': () => ({
    problem: 'Sorted array mein X sahi jagah insert karo order maintain karke.',
    approach: ['Pehle insert position dhundho (jahan arr[i] > X)', 'Right shift from that index', 'Insert X'],
  ...
  'array-separate-even-odd': () => ({
    ...
    approach: ['Two pointers ya extra array', 'Evens collect, phir odds', 'Ya partition like quicksort'],
  ...
  'array-longest-increasing-run': () => ({
    ...
    approach: ['currentRun=1, best=1', 'Har i: if arr[i]>arr[i-1]: currentRun++, else reset 1', 'best = max(best, currentRun)'],
    solution: 'Sliding streak counter — O(n).',
  }),
  'array-pair-sum': () => ({
    problem: 'Sorted array mein do numbers jinka sum = target.',
    approach: ['left=0, right=n-1', 'sum = arr[left]+arr[right]', 'sum<target → left++, sum>target → right--', 'Equal → pair mila'],
  ...
  'array-count-pairs-sum': () => ({
    ...
    approach: ['Two pointer jaisa pair-sum', 'Duplicate values par count logic', 'Ya hash map complements'],
    solution: 'Sorted: two pointer with skip duplicates. O(n).',
  }),
  'array-all-pairs-sum': () => ({
    problem: 'Saare pairs print jinka sum target.',
    approach: ['Two pointer ya nested loop', 'Har valid pair collect/print'],
    solution: 'Two pointer O(n) sorted; nested O(n²) general.',
  }),
  'array-triplet-sum': () => ({
    problem: 'Teen numbers jinka sum target (sorted array).',
    approach: ['Fix i, then two pointer on i+1..n-1 for pair sum = target-arr[i]'],
  ...
  'array-count-pairs-diff': () => ({
    ...
    approach: ['Hash map: har element ke liye count (x-K) and (x+K) carefully', 'Ya sort + two pointer'],
  ...
  'array-max-sum-k-window': () => ({
    ...
    approach: ['Pehle window [0..K-1] ka sum', 'Slide: naya add arr[i], purana arr[i-K] subtract', 'Har window par max update'],
  ...
  'array-min-sum-k-window': () => ({
    ...
    approach: ['Same sliding window, min track instead of max'],
  ...
  'array-first-negative-window': () => ({
    ...
    approach: ['Queue/index list of negatives in window', 'Slide window, expire old negatives', 'Front = first negative'],
    solution: 'Deque of indices — O(n). Naive O(n*k).',
  }),
  'array-max-every-window': () => ({
    problem: 'Har window size K ka maximum element.',
    approach: ['Monotonic deque: decreasing order mein indices', 'Front = max for current window'],
  ...
  'array-count-subarrays-sum-k': () => ({
    ...
    approach: ['Prefix sum + hashmap', 'Har prefix P: count how many (P-K) seen before'],
  ...
  'array-subarray-given-sum': () => ({
    ...
    approach: ['Two pointer / sliding window', 'Expand right, shrink left jab sum > target'],
  ...
  'array-longest-subarray-sum-k': () => ({
    ...
    approach: ['Prefix sum + map: earliest index for each prefix', 'max len for prefix-K'],
    solution: 'Hash map stores first occurrence of prefix — O(n).',
  }),
  'array-kadane': () => ({
    problem: 'Maximum subarray sum (contiguous, Kadane).',
    approach: ['currentSum = 0, maxSum = -inf', 'Har x: currentSum = max(x, currentSum+x)', 'maxSum = max(maxSum, currentSum)'],
  ...
  'array-majority-boyer-moore': () => ({
    ...
    approach: ['candidate, count=0', 'Same → count++, different → count--, zero → new candidate', 'Verify candidate count'],
    solution: 'O(n) time O(1) space voting — cancel pairs.',
  }),
  'array-stock-single': () => ({
    problem: 'Ek buy ek sell — max profit.',
    approach: ['minPrice track (best buy so far)', 'Har day: profit = price - minPrice, maxProfit update', 'price min update'],
    solution: 'One pass O(n) — min so far + max profit.',
  }),
  'array-stock-multiple': () => ({
    problem: 'Unlimited transactions — max profit.',
    approach: ['Har rise capture: sum (price[i]-price[i-1]) jahan positive', 'Ya valley-peak pairs add'],
  ...
  'array-container-water': () => ({
    ...
    approach: ['left=0, right=n-1', 'Area = min(h[l],h[r])*(r-l)', 'Chhote height wala pointer move karo'],
    solution: 'Two pointer greedy — O(n). Move shorter line inward.',
  }),
  'array-max-difference': () => ({
    problem: 'Max arr[j]-arr[i] where j>i.',
    approach: ['minSoFar track', 'Har j: maxDiff = max(maxDiff, arr[j]-minSoFar)', 'minSoFar update'],
    solution: 'Stock problem jaisa — O(n).',
  }),
  'array-smallest-missing-positive': () => ({
    problem: 'Sabse chhota missing positive integer (1,2,3…).',
    approach: ['Cyclic sort: value v ko index v-1 par rakho', 'Phir scan first wrong index+1'],
    solution: 'Index as hash O(n) O(1) — place each num at correct slot.',
  }),
  'array-bubble-sort': () => ({
    problem: 'Bubble sort — adjacent swap se sort.',
    approach: ['n passes, har pass mein largest bubble end tak', 'if arr[j]>arr[j+1] swap', 'Early exit if no swap'],
  ...
  'array-peak-binary-search': () => ({
    ...
    approach: ['mid compare arr[mid+1]', 'Uphill → left half mein peak, else right'],
...
lines.push('};', '');
lines.push('export function getAlgorithmForId(id: string): AlgoBlock | undefined {');
lines.push('  return ALGORITHM_BY_ID[id];');
lines.push('}', '');

const outPath = path.join(root, 'src/explain/algorithmCatalog.ts');
fs.writeFileSync(outPath, lines.join('\n'));
console.log(`Wrote ${problems.length} algorithms to ${outPath}`);
